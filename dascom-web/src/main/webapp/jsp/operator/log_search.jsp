  ...
  if(fm.startTime.value != '') {
	...
	if(fm.stopTime.value != '') {
		if(!isDate(fm.stopTime.value)){
			wBox=$("#wbox2").wBox({
			title: "返回结果",
			html: "<div style='width:300px;height:100px;'><table align='center' style='margin-top:40px;width:100%;'><tr><td align='center'>终止时间不正确，请重新输入！</td></tr></table></div>"
			});
			wBox.showBox();
			fm.stopTime.focus();
			return false;
...
{ 
  if(check()){
  document.mainfm.action="OPexportFile.do?flag=1";
...
{ 
  if(check()){
  document.mainfm.action="logSearch.do";
  document.mainfm.submit();
  }
}
function cleElements()
{
   document.mainfm.opeNo.value="";
...
<div id="title" style="height:20px"><img src="images/500000002.gif" width="10" height="11" />&nbsp;当前位置：日志信息查询</div>
		<hr/>
		<br/>
		<form method="post" action="logSearch.do" onSubmit="return check();"
			name="mainfm">
			<IFRAME id="CalFrame"
				style="DISPLAY: none; Z-INDEX: 100; WIDTH: 148px; POSITION: absolute; HEIGHT: 194px; left: 42px; top: 50px"
				marginWidth="0" marginHeight="0" src="scripts/calendar/calendar.htm"
				frameBorder="0" noResize scrolling="no"></IFRAME>
			<table align="center" width="90%">
				<tr>
					...
					<td class="t7">
						<input type="text" name="opeNo" maxlength="16" value="${opeNo}"/>
					</td>
				</tr>
					...
					<td class="t7">
						<select name="logType">
							...
							<c:forEach items="${operationType}" var="string">
								<option value="${string}"
									<c:if test="${logType==string}">selected</c:if>>
									${string}
								</option>
							</c:forEach>
						</select>
				...
				<tr>
					...
					<td class="t7">
						从
						<input type="text" name="startTime" maxlength="10" size="12"
							value="${startTime}" onfocus="WdatePicker({firstDayOfWeek:1,readOnly:true,
							dateFmt:'yyyy-MM-dd'})" class="Wdate" style="width:150px"/>
						到
						<input type="text" name="stopTime" maxlength="10" size="12"
							value="${stopTime}" onfocus="WdatePicker({firstDayOfWeek:1,readOnly:true,
							dateFmt:'yyyy-MM-dd'})" class="Wdate" style="width:150px"/>
						<font color="red">*</font>
					</td>
				</tr>
				...
				<tr>
					<td height="40" colspan="2" align="center">
						<input class="btnstyle" onmouseout="this.style.backgroundImage='url(images/btnbg.png)';" onmouseover="this.style.backgroundImage='url(images/btnbg2.png)';" type="button" value="查询" onClick="log_search();"/>&nbsp;&nbsp;&nbsp;
       					<input class="btnstyle" onmouseout="this.style.backgroundImage='url(images/btnbg.png)';" onmouseover="this.style.backgroundImage='url(images/btnbg2.png)';" type="button" value="清除" onClick="cleElements();"/>&nbsp;&nbsp;&nbsp;
		...
		<c:if test="${logList!=null}">
			...
			<form method="post" action="logSearch.do" name="pageForm">
				...
				<common:pageLocator recordCount="${recordCount}"
					pageSize="${pageSize}" currentPage="${pageNum}" formName="pageForm"
					url="logSearch.do?page=" />
			</form>
		</c:if>
		<script language="JavaScript" type="text/JavaScript">
<c:if test="${logType != null and logType != ''}">document.all.mainfm.logType.value="${logType}";
</c:if>
</script>
	</body>
</html>
