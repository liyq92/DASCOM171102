...
*修改人:Andy
*日期：2003-03-20
*修改内容：1.增加 HideCalendar()函数
*         2.增加 setFormatDate(obj)函数,将时间输入框中时间字符串自动转换成适应数据库的格式
***************************************/
...
function HideCalendar(){
  var dF=document.all.CalFrame;
  if(event.keyCode==9){      //TAB键按下
    dF.style.display="none";
  }
}

/***************************************
*功能：将时间输入框中时间字符串自动转换成适应数据库的格式
*开发: Andy
*日期：2003-03-20
***************************************/
function setFormatDate(obj){
 ...
 var formatedDate = getFormatDate(objvalue, strFMT);
 if(formatedDate != 0){
   obj.value=formatedDate;
   //HideCalendar();
 }
 else{
   alert("您输入的不是一个有效的日期格式\n有效的日期格式如下:\n'2003-03-21', '2003/03/21'\n'2003.03.21', '03/21/2003'\n'2003.1.1',     '2003/1/1'");
   obj.value='';
   obj.focus();
 }
}
